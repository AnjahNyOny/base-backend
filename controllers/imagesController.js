...
export async function createImage(req, res) {
  try {
    ...
    if (!contenu_id) {
      return res.status(400).json({ message: "Champ requis: 'contenu_id'." });
    }

    const url = (image_url ?? '').trim();
    const a   = (alt ?? '').trim();

    // ✅ Assouplissement: autoriser alt-only si c'est une icône
    if (!url && !isIconToken(a)) {
      return res.status(400).json({
        message: "Champs requis: fournir 'image_url' (image) ou 'alt' commençant par 'icon:' (icône)."
      });
    }

    const [result] = await db.query(
      'INSERT INTO ContenuImage (contenu_id, image_url, alt) VALUES (?, ?, ?)',
      [contenu_id, url || null, a || null]
    );
    res.status(201).json({
      id: result.insertId,
...
export async function updateImage(req, res) {
  try {
    ...
    // ✅ Assouplissement: accepter 'image_url' vide si alt = icon:…
    if (!url && !isIconToken(a)) {
      return res.status(400).json({
        message: "Champ requis: 'image_url' (image) ou 'alt' commençant par 'icon:' (icône)."
      });
    }
    ...
    res.json({ id: Number(id), image_url: url || null, alt: a || null });
  } catch (err) {
