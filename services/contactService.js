...
//     const like = `%${q.trim()}%`;
//     where.push(`(
//       t.subject LIKE ? OR t.user_email LIKE ? OR t.user_name LIKE ?
//       OR EXISTS (
//         SELECT 1
...
//   // --- Parse q pour extraire has:attachment ---
//   let query = (q || "").toString().trim();
//   let hasAttachment = false;
//   if (query) {
//     const parts = query.split(/\s+/);
//     const rest = [];
//     for (const part of parts) {
//       if (part.toLowerCase() === "has:attachment") hasAttachment = true;
//       else rest.push(part);
//     }
...
//   // --- Status UI -> DB (avec waiting pris comme "new" pour la vue "Nouveaux") ---
//   const dbStatus = uiToDbStatus(status);
//   if (dbStatus) {
//     if (dbStatus === "new") {
//       // si tu conserves encore "waiting" en base
//       where.push(`t.status IN ('new','waiting')`);
//     } else {
//       where.push(`t.status = ?`);
//       args.push(dbStatus);
//     }
...
//       EXISTS (
//         SELECT 1
//           FROM contact_messages cma
//          WHERE cma.thread_id = t.id
//            AND cma.attachments IS NOT NULL
//            AND cma.attachments <> ''
//            AND JSON_LENGTH(cma.attachments) > 0
//       )
//     `);
//   }
...
//       /* premier message entrant (pour le preview) */
//       im.body_text AS first_message,
...
//       /* nombre total de pièces jointes sur le thread */
//       COALESCE((
//         SELECT SUM(JSON_LENGTH(cm2.attachments))
