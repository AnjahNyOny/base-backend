...
import db from "../config/db.js";
import { inboundCreatedTotal } from "./metrics.js";

// --- IMPORT DU NOUVEAU SERVICE EMAIL (Pour le style) ---
import { sendAndPersistOutbox } from "./emailService.js";
...
export async function createInboundMessage({
  ...
  // Nettoyage minimal des pièces jointes
  const cleanAttachments = Array.isArray(attachments)
    ? attachments.map((a) => ({
      name: a?.name ?? a?.filename ?? null,
      mime: a?.mime ?? a?.contentType ?? null,
...
function parseSearch(qRaw = "") {
  ...
  while ((m = re.exec(q))) {
    ...
    else if (key === "has:attachment") ops.hasAttachment = true;
    else terms.push(`${key}:${val}`);
  }
...
export async function listThreads({ q, page = 1, pageSize = 20, status } = {}) {
  ...
  if (ops.id.length) {
    const ids = ops.id.map(v => Number(v)).filter(n => Number.isFinite(n));
    if (ids.length) {
      where.push(`t.id IN (${ids.map(() => "?").join(",")})`);
      args.push(...ids);
    }
  ...
  if (ops.before) {
    where.push(`GREATEST(COALESCE(t.last_incoming_at, '1970-01-01'), COALESCE(t.last_outgoing_at, '1970-01-01'), t.updated_at) <= ?`);
    args.push(dayEnd(ops.before));
  }
  if (ops.after) {
    where.push(`GREATEST(COALESCE(t.last_incoming_at, '1970-01-01'), COALESCE(t.last_outgoing_at, '1970-01-01'), t.updated_at) >= ?`);
  ...
  if (ops.hasAttachment) {
    where.push(`EXISTS (SELECT 1 FROM contact_messages cm_att WHERE cm_att.thread_id = t.id AND JSON_LENGTH(cm_att.attachments) > 0)`);
  }
  if (ops.label.length) {
    where.push(`EXISTS (SELECT 1 FROM contact_thread_labels ctl JOIN contact_labels cl ON cl.id = ctl.label_id WHERE ctl.thread_id = t.id AND ( ${ops.label.map(() => "cl.name LIKE ?").join(" OR ")} ))`);
    args.push(...ops.label.map(v => `%${v}%`));
  }
  if (terms.length) {
    for (const tTerm of terms) {
      const like = `%${tTerm}%`;
      where.push(`(t.subject LIKE ? OR t.user_email LIKE ? OR t.user_name LIKE ? OR EXISTS (SELECT 1 FROM contact_messages cmq WHERE cmq.thread_id = t.id AND (cmq.body_text LIKE ? OR cmq.body_html LIKE ?)))`);
      args.push(like, like, like, like, like);
    }
  }

  const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";
  const orderExpr = `GREATEST(COALESCE(t.last_incoming_at, '1970-01-01'), COALESCE(t.last_outgoing_at, '1970-01-01'), t.updated_at) DESC`;

  const [rows] = await db.query(
    `SELECT t.id, t.subject, t.user_name, t.user_email, t.status, t.priority, t.langue, t.created_at, t.updated_at, t.last_incoming_at, t.last_outgoing_at, im.body_text AS first_message,
      (SELECT COUNT(*) FROM contact_messages cm WHERE cm.thread_id = t.id) AS message_count,
      COALESCE((SELECT GROUP_CONCAT(cl.name ORDER BY cl.name SEPARATOR ',') FROM contact_thread_labels ctl JOIN contact_labels cl ON cl.id = ctl.label_id WHERE ctl.thread_id = t.id), '') AS labels
    FROM contact_threads t
    LEFT JOIN (SELECT cm.thread_id, cm.body_text FROM contact_messages cm JOIN (SELECT thread_id, MIN(id) AS min_id FROM contact_messages WHERE direction = 'in' GROUP BY thread_id) f ON f.thread_id = cm.thread_id AND f.min_id = cm.id) im ON im.thread_id = t.id
    ${whereSql} ORDER BY ${orderExpr} LIMIT ? OFFSET ?`,
    [...args, ps, off]
  );
...
export async function bulkSetStatus(ids = [], uiStatus) {
  ...
  const placeholders = ids.map(() => "?").join(",");
  const [res] = await db.query(
    `UPDATE contact_threads SET status = ?, updated_at = ? WHERE id IN (${placeholders})`,
    [dbStatus, nowMySQL(), ...ids]
  );
  return res.affectedRows || 0;
}
...
/* ===================================================================
   REPLY (ADMIN) -> C'est ici qu'on branche le nouveau service
   =================================================================== */
export async function replyToThread(
  threadId,
  { subject, message, sender_name, sender_email, attachments = [] }
) {
  const id = Number(threadId);
  if (!Number.isFinite(id)) throw new Error("threadId invalide.");
  ...
  // 1. Récupération des infos du Thread
  const [[t] = []] = await db.query(
    `SELECT user_email, user_name, status FROM contact_threads WHERE id = ? LIMIT 1`,
    [id]
  );
  ...
  // 2. Préparation du HTML pour le template
  // On transforme les retours à la ligne en balises <p> pour que ce soit joli
  const htmlBody = text
    .split('\n')
  ...
  try {
    ...
    // 3. Sauvegarde dans l'historique de chat Admin (contact_messages)
    // On sauvegarde le text brut pour l'affichage admin simple
    // Et le HTML généré si besoin
    const [mRes] = await cnx.query(
      `INSERT INTO contact_messages
         (thread_id, direction, sender_email, sender_name, body_text, body_html, attachments, created_at)
       VALUES
         (?, 'out', ?, ?, ?, ?, ?, ?)`,
      [id, adminEmail, adminName, text, htmlBody, JSON.stringify(attachments), now]
    );
    // const messageId = mRes.insertId; // On pourrait lier cet ID à l'outbox si on voulait

    // 4. Mise à jour du Thread (statut ouvert, date update)
    await cnx.query(
      `UPDATE contact_threads
         SET last_outgoing_at = ?, updated_at = ?,
             status = CASE WHEN status = 'new' THEN 'open' ELSE status END
...
export async function upsertLabel({ name }) {
  ...
  if (!n) throw new Error("name requis.");
  const [res] = await db.query(
    `INSERT INTO contact_labels (name) VALUES (?) ON DUPLICATE KEY UPDATE name = VALUES(name)`,
    [n]
  );
  return res.insertId || true;
}
export async function removeLabel(id) {
  ...
  try {
    ...
    await cnx.commit();
    return res.affectedRows > 0;
  } catch (e) { await cnx.rollback(); throw e; } finally { cnx.release(); }
}
export async function setThreadLabels(threadId, labelIds = []) {
  const id = Number(threadId);
  const cnx = await db.getConnection();
